...
namespace IvanAkcheurov.NTextCat.Lib
{
    public class TextReaderStream : Stream
    {
        ...
        public TextReaderStream(TextReader textReader, Encoding encoding, int chunkChars = 4096)
        {
            _textReader = textReader;
            _encoding = encoding;
            _maxByteCountPerChar = _encoding.GetMaxByteCount(1);
            _encoder = encoding.GetEncoder();
            if (chunkChars <= 0) throw new ArgumentOutOfRangeException("chunkChars", "zero or negative");
            _charBuffer = new char[chunkChars];
        }
